...
// Markdown and never looked at the SQL built from it. That is not the same
// check, and the difference was not academic: seed_content.sql sat 2,109
// lines behind content/ with CI green the whole time, because eight reviewed
// Module 5 lessons had been written and the seed never regenerated. Anybody
// deploying from this repo would have loaded 42 lessons and had no way to
// know 50 existed.
//
// So generation now always runs, and --check compares the result against
// what is committed instead of exiting early. A generated file is only
// trustworthy if something fails when it drifts.
...
const artifacts = [{ path: OUT, body: out.join('\n') }]
...
// A chunk left behind by a shrinking seed is not clutter: the instruction is
// "run them in order", so a stale trailing chunk re-inserts content that the
// source no longer has.
const expected = new Set(artifacts.map((a) => a.path))
const stale = existsSync(CHUNKS)
  ? readdirSync(CHUNKS)
...
if (CHECK_ONLY) {
  const drifted = artifacts.filter(
    (a) => !existsSync(a.path) || readFileSync(a.path, 'utf8') !== a.body,
  )
  if (drifted.length > 0 || stale.length > 0) {
    console.error(
      `\nGenerated SQL is out of date with content/:\n` +
        drifted
          .map((a) => `  - ${a.path.slice(ROOT.length + 1)} (stale or missing)`)
          .join('\n') +
        (drifted.length && stale.length ? '\n' : '') +
        stale
          .map((f) => `  - ${f.slice(ROOT.length + 1)} (no longer generated)`)
          .join('\n') +
  ...
  console.log(
    `\nsupabase/seed_content.sql and ${chunks.length} chunk(s): up to date`,
  )
} else {
  ...
  if (stale.length > 0) {
    console.log(
      `\nStale chunk files, delete them:\n` +
        stale.map((f) => `  - ${f.slice(ROOT.length + 1)}`).join('\n'),
    )
  }
}
